...
public class SubProjectHandler implements EventHandler{
    ...
    //Create Method of SubProjects
 
    // @Before(event = CqnService.EVENT_CREATE, entity = "ManagementService.SubProjects")
    ...
    //     try {
    //         int moduleId = generateNewId("ManagementService.SubProjects", "moduleId");
    //         projectData.put("moduleId", moduleId);
    //         System.out.println("Generated project ID: " + moduleId);
    //     } catch (Exception e) {
    //         System.err.println("Error generating module ID: " + e.getMessage());
    //         throw new RuntimeException("Error generating module ID", e);
    //     }
    // }
 
    @On(event = CqnService.EVENT_CREATE, entity = "ManagementService.SubProjects")
    public void createProject(CdsCreateEventContext context, CdsData projectData) {
        ...
        try {
            ...
            db.run(Insert.into("ManagementService.SubProjects").entry(projectData));
            System.out.println("SubProject inserted successfully with ID: " + moduleId);
 
            context.setResult(Collections.singletonList(projectData));
        } catch (Exception e) {
            System.err.println("Error creating SubProjects: " + e.getMessage());
            throw new RuntimeException("Error creating SubProjects: " + e.getMessage(), e);
        }
    }
